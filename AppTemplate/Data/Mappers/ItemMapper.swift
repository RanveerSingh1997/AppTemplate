...
struct ItemMapper: EntityMapper {
    func toEntity(dto: ItemDTO, context: ModelContext) -> Result<CachedItem, AppError> {
        let entity = CachedItem(id: dto.id, title: dto.name, detail: dto.description)
        entity.markInserted()
    ...
    func toDTO(entity: CachedItem) -> Result<ItemDTO, AppError> {
        .success(ItemDTO(id: entity.id, name: entity.title, description: entity.detail))
    }
...
extension ItemDTO {
    var asDomain: Item { Item(id: id, title: name, detail: description) }
}

extension Item {
    var asDTO: ItemDTO { ItemDTO(id: id, name: title, description: detail) }
}
